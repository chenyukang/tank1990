...
const VIEW_3D_BRICK_HEIGHT: f32 = 5.0;
const VIEW_3D_STEEL_HEIGHT: f32 = 7.0;
const VIEW_3D_FOREST_HEIGHT: f32 = 4.0;
const VIEW_3D_TANK_HEIGHT: f32 = 4.0;
const VIEW_3D_BASE_HEIGHT: f32 = 8.0;
...
const VIEW_3D_PROTECTION_HEIGHT: f32 = 6.2;
...
const VIEW_3D_CAMERA_DISTANCE: f32 = 34.0;
const VIEW_3D_CAMERA_HEIGHT: f32 = 23.0;
const VIEW_3D_CAMERA_LOOK_AHEAD: f32 = 24.0;
const VIEW_3D_CAMERA_LOOK_HEIGHT: f32 = 3.0;
const VIEW_3D_OCCLUDED_CAMERA_DISTANCE: f32 = 26.0;
const VIEW_3D_OCCLUDED_CAMERA_HEIGHT: f32 = 42.0;
const VIEW_3D_OCCLUDED_LOOK_HEIGHT: f32 = 8.0;
...
const VIEW_3D_MINIMAP_LEFT: f32 = VIRTUAL_WIDTH - VIEW_3D_MINIMAP_SIZE as f32 - 6.0;
const VIEW_3D_MINIMAP_TOP: f32 = 6.0;
...
pub(super) fn setup_3d_view(
    ...
    commands.spawn((
        ...
        Transform::from_xyz(0.0, 115.0, 150.0).looking_at(Vec3::ZERO, Vec3::Y),
        RenderLayers::layer(VIEW_3D_LAYER),
        Tank3dCamera,
    ));
    commands.spawn((
...
fn create_3d_assets(
    ...
    View3dAssets {
        ...
        wall_mesh: meshes.add(Cuboid::new(TILE_SIZE, 1.0, TILE_SIZE)),
        ...
        protection_x_mesh: meshes.add(Cuboid::new(18.0, 0.8, 1.2)),
        protection_z_mesh: meshes.add(Cuboid::new(1.2, 0.8, 18.0)),
        ...
        ground_material: materials.add(Color::srgb_u8(24, 32, 28)),
        brick_material: materials.add(Color::srgb_u8(152, 64, 36)),
        steel_material: materials.add(Color::srgb_u8(136, 144, 152)),
        forest_material: materials.add(StandardMaterial {
            base_color: Color::srgba_u8(56, 132, 72, 190),
        ...
        ice_material: materials.add(Color::srgb_u8(184, 232, 248)),
        player_one_material: materials.add(Color::srgb_u8(184, 248, 184)),
        player_two_material: materials.add(Color::srgb_u8(136, 216, 255)),
        ...
        enemy_basic_material: materials.add(Color::srgb_u8(232, 232, 216)),
        enemy_fast_material: materials.add(Color::srgb_u8(112, 216, 128)),
        enemy_power_material: materials.add(Color::srgb_u8(248, 112, 112)),
        enemy_armor_material: materials.add(Color::srgb_u8(176, 176, 216)),
        barrel_material: materials.add(Color::srgb_u8(48, 56, 48)),
        base_material: materials.add(Color::srgb_u8(232, 192, 88)),
        player_one_base_material: materials.add(Color::srgb_u8(144, 248, 152)),
        player_two_base_material: materials.add(Color::srgb_u8(112, 184, 255)),
        ...
        shield_material: materials.add(StandardMaterial {
            base_color: Color::srgba_u8(112, 216, 248, 220),
            alpha_mode: AlphaMode::Blend,
            unlit: true,
            ..default()
        }),
        spawn_protection_material: materials.add(StandardMaterial {
            base_color: Color::srgba_u8(232, 248, 255, 225),
            alpha_mode: AlphaMode::Blend,
            unlit: true,
            ..default()
        }),
...
pub(super) fn handle_view_hotkeys(
    ...
    if keys.just_pressed(KeyCode::KeyV) && game_status.phase != GamePhase::ModeSelect {
...
pub(super) fn update_3d_chase_camera(
    ...
    let Some((tank, _player)) = target_player else {
    ...
    let camera_transform = chase_camera_transform(tank, &tile_grid);
    for mut transform in &mut cameras {
...
pub(super) fn sync_3d_hud(
    ...
    if let (Some(view_assets), Some(images)) = (view_assets, images.as_deref_mut()) {
        ...
        spawn_3d_minimap(&mut commands, &view_assets);
    }
}
...
fn spawn_3d_minimap(commands: &mut Commands, assets: &View3dAssets) {
    let size = Vec2::splat(VIEW_3D_MINIMAP_SIZE as f32);
    spawn_3d_hud_panel(
        commands,
        Vec2::new(VIEW_3D_MINIMAP_LEFT - 3.0, VIEW_3D_MINIMAP_TOP - 3.0),
        size + Vec2::splat(6.0),
    );
    commands.spawn((
        Sprite::from_image(assets.minimap_image.clone()),
        Transform::from_translation(virtual_center_scaled(
            Vec2::new(VIEW_3D_MINIMAP_LEFT, VIEW_3D_MINIMAP_TOP),
            size,
            VIEW_3D_HUD_TEXT_Z,
        ))
...
fn spawn_3d_tile(
    ...
    match tile {
        ...
        TileKind::Brick => spawn_3d_block_tile(
            commands,
            assets,
            x,
            y,
        ...
        TileKind::Steel => spawn_3d_block_tile(
            commands,
            assets,
            x,
            y,
        ...
        TileKind::Forest => spawn_3d_block_tile(
            commands,
            assets,
            x,
            y,
            VIEW_3D_FOREST_HEIGHT,
            assets.forest_material.clone(),
        ),
        TileKind::Water => {
            spawn_3d_floor_tile(commands, assets, x, y, assets.water_material.clone())
        }
        TileKind::Ice => spawn_3d_floor_tile(commands, assets, x, y, assets.ice_material.clone()),
    }
}

fn spawn_3d_block_tile(
    commands: &mut Commands,
    assets: &View3dAssets,
    x: usize,
    y: usize,
    ...
    let mut transform =
        Transform::from_translation(board_3d_center(top_left, Vec2::splat(TILE_SIZE), height));
...
fn spawn_3d_tank_protection(
    ...
    let center = board_3d_center(top_left, Vec2::splat(TANK_SIZE), VIEW_3D_PROTECTION_HEIGHT);
    let y_offset = Vec3::Y * 1.5;
    for (mesh, offset) in [
        (assets.protection_x_mesh.clone(), Vec3::new(0.0, 0.0, -9.0)),
        (assets.protection_x_mesh.clone(), Vec3::new(0.0, 0.0, 9.0)),
        (assets.protection_z_mesh.clone(), Vec3::new(-9.0, 0.0, 0.0)),
        (assets.protection_z_mesh.clone(), Vec3::new(9.0, 0.0, 0.0)),
    ] {
        spawn_3d_mesh(
            commands,
            mesh,
            material.clone(),
            Transform::from_translation(center + y_offset + offset),
            View3dDynamic,
        );
    }
}
...
pub(super) fn chase_camera_transform(tank: &Tank, tile_grid: &TileGrid) -> Transform {
    ...
    let forward_2d = tank.facing.movement();
    let desired_camera_ground = target_center - forward_2d * VIEW_3D_CAMERA_DISTANCE;
    let occluded = camera_path_is_obstructed(tile_grid, target_center, desired_camera_ground);

    let (distance, camera_height, look_height) = if occluded {
        (
...
fn tank_barrel_transform(top_left: Vec2, direction: Direction) -> (BarrelMesh, Vec3) {
    ...
    let barrel_center = center + forward * 7.0 + Vec3::Y * 1.0;
